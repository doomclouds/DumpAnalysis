...
> 目标：从零学会用 **WinDbg MCP**（`mcp-windbg`，驱动 `cdb.exe` + SOS）分析 .NET 进程的 dump。读完这篇，你就能对仓库里 6 个经典场景做完整分析。

---

## 1. 这是什么

WinDbg MCP 是一个 MCP 服务器，它把微软调试器 `cdb.exe` 包装成一组可调用的工具，让你在 Claude Code 里直接对 Windows 崩溃/内存转储（.dmp）执行调试命令。对 .NET 来说，关键是加载 **SOS 扩展**——它暴露了 `!dumpheap`、`!gcroot`、`!finalizequeue` 等专查托管堆的命令。
...
| `mcp-windbg` MCP 服务器 | 已接入 Claude Code 的工具 | 会话里有 `mcp__mcp-windbg__*` 工具 |
